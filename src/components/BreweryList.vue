    ...
    <ion-header collapse="condense">
      <ion-toolbar>
        <ion-title data-test="app-header" size="large"
          >Hoppy To Be-Er | Breweries</ion-title
        >
      </ion-toolbar>
    </ion-header>
...
const fetchBreweries = async (): Promise<void> => {
  loading.value = true;
  try {
    breweries.value = await api.fetchBreweries(
      currentPage.value,
      perPage.value,
      searchQuery.value,
      breweryType.value,
    ...
    if (!searchQuery.value) {
      ...
      totalBreweries.value =
        (currentPage.value - 1) * perPage.value + receivedCount;
...
const goToPage = (page: number): void => {
  router.push({
    query: { ...route.query, page: page.toString(), search: searchQuery.value },
  }); // Update query params
...
ion-content::part(scroll) {
  padding: var(--ion-safe-area-top, 0) var(--ion-safe-area-right, 0)
    var(--ion-safe-area-bottom, 0) var(--ion-safe-area-left, 0);
}
</style>
